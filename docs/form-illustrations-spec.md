...
## フォーム注釈(FormAnnotations)の必須検品プロセス

> **背景**: 2026-07-13、注釈座標の一部が「部位ではなく地面や説明と異なる箇所を
> 指している」問題が実際に発生した(例: air-squat の「踵」ドットが地面、
> reverse-lunge の「後ろ膝」が床の白線上)。座標は写真を見ずに書くと必ずずれる。

`WorkoutKit/Resources/FormAnnotations/<slug>-annotations.json` を追加・変更する
場合、以下の検品を**必ず**行うこと(機械チェックだけでは不十分):

1. **視覚照合(必須)**: 対象写真をビューアで開き、各 `position`(正規化 0..1、
   x=左から / y=上から)をピクセル換算して、ドットが**説明文の部位の上**に
   乗っていることを 1 点ずつ確認する。「だいたい近く」は不合格。地面・背景・
   隣の部位は不合格。
2. **吹き出し位置**: `labelAnchor` は被写体に重ならない空き領域(背景・床)に
   置く。0.08..0.92 の範囲内。
3. **引き出し線の横断チェック**: ドット→アンカーの直線が**他の部位**を横切らない
   こと(注釈対象の部位自体を通るのは可)。
4. **機械チェック**: JSON が parse できる / labelKey が Localizable.xcstrings に
   存在する / assetName の imageset が存在する。
5. **両テーマ確認**: 吹き出しはすりガラスなので、明るい写真・暗い写真の両方で
   文字が読めることをプレビューで確認。

検品記録はコミットメッセージに「座標検品済み(視覚照合)」と明記する。
